import json

import jwt
from django.conf import settings
from django.http import JsonResponse
from django.views.decorators.csrf import csrf_exempt

from .models import Payment


def _bad_request(message: str, status: int = 400) -> JsonResponse:
    return JsonResponse({"error": message}, status=status)


def _parse_json(request):
    if not request.body:
        return {}, None
    try:
        return json.loads(request.body), None
    except json.JSONDecodeError:
        return None, _bad_request("Invalid JSON")


def _decode_token(token: str) -> dict | None:
    try:
        return jwt.decode(token, settings.SECRET_KEY, algorithms=["HS256"])
    except jwt.PyJWTError:
        return None


def _get_authenticated_user(request):
    auth_header = request.headers.get("Authorization", "")
    if not auth_header.startswith("Bearer "):
        return None, _bad_request("Missing token", status=401)

    token = auth_header.split(" ", 1)[1].strip()
    payload = _decode_token(token)
    if not payload or payload.get("type") != "access":
        return None, _bad_request("Invalid token", status=401)

    return payload, None


def _require_role(payload: dict, roles: set[str]):
    if payload.get("role") not in roles:
        return _bad_request("Forbidden", status=403)
    return None


def _serialize_payment(payment: Payment) -> dict:
    return {
        "id": payment.id,
        "order_id": payment.order_id,
        "amount": payment.amount,
        "status": payment.status,
    }


@csrf_exempt
def pay(request):
    if request.method != "POST":
        return _bad_request("Method not allowed", status=405)

    payload, auth_error = _get_authenticated_user(request)
    if auth_error:
        return auth_error

    role_error = _require_role(payload, {"customer"})
    if role_error:
        return role_error

    data, error = _parse_json(request)
    if error:
        return error

    try:
        order_id = int(data.get("order_id"))
        amount = float(data.get("amount"))
    except (TypeError, ValueError):
        return _bad_request("Invalid order_id or amount")

    payment = Payment.objects.create(
        order_id=order_id,
        amount=amount,
        status=Payment.STATUS_PENDING,
    )

    return JsonResponse(_serialize_payment(payment), status=201)


@csrf_exempt
def payment_status(request, order_id: int):
    if request.method != "GET":
        return _bad_request("Method not allowed", status=405)

    payload, auth_error = _get_authenticated_user(request)
    if auth_error:
        return auth_error

    role_error = _require_role(payload, {"customer", "staff"})
    if role_error:
        return role_error

    payment = Payment.objects.filter(order_id=order_id).order_by("-id").first()
    if not payment:
        return _bad_request("Payment not found", status=404)

    return JsonResponse(_serialize_payment(payment))


@csrf_exempt
def refund(request, payment_id: int):
    if request.method != "POST":
        return _bad_request("Method not allowed", status=405)

    payload, auth_error = _get_authenticated_user(request)
    if auth_error:
        return auth_error

    role_error = _require_role(payload, {"admin"})
    if role_error:
        return role_error

    payment = Payment.objects.filter(id=payment_id).first()
    if not payment:
        return _bad_request("Payment not found", status=404)

    payment.status = Payment.STATUS_FAILED
    payment.save(update_fields=["status"])
    return JsonResponse(_serialize_payment(payment))


@csrf_exempt
def payment_history(request):
    if request.method != "GET":
        return _bad_request("Method not allowed", status=405)

    payload, auth_error = _get_authenticated_user(request)
    if auth_error:
        return auth_error

    role_error = _require_role(payload, {"admin"})
    if role_error:
        return role_error

    payments = Payment.objects.all().order_by("-id")
    data = [_serialize_payment(payment) for payment in payments]
    return JsonResponse(data, safe=False)
